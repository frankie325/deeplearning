...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="image",
        type=str,
        help="Use the data from image, video or camera",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--path_to_img",
        default="dataset/demo/images/",
        type=str,
        help="The path to image files",
    )
    parser.add_argument(
        "--path_to_vid",
        default="dataset/demo/videos/",
        type=str,
        help="The path to video files",
    )
    parser.add_argument(
        "--path_to_save",
    ...
    parser.add_argument(
        "--show", action="store_true", default=False, help="show visualization"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-nc", "--num_classes", default=80, type=int, help="number of classes."
    )
    parser.add_argument(
        "--weight",
        default=None,
    ...
    parser.add_argument(
        "--topk", default=100, type=int, help="topk candidates for testing"
    )
    parser.add_argument(
        "--deploy",
        action="store_true",
...
def detect(args, model, device, transform, vis_thresh, mode="image"):
    # class color
    np.random.seed(0)
    class_colors = [
    ...
    if mode == "camera":
        ...
        if args.gif:
            ...
            save_gif_name = os.path.join(
                save_gif_path, "{}.gif".format(cur_time)
            )
    ...
    # ------------------------- Image ----------------------------
        ...
        for i, img_id in enumerate(os.listdir(args.path_to_img)):
            image = cv2.imread(
                (args.path_to_img + "/" + img_id), cv2.IMREAD_COLOR
            )
            ...
            cv2.imwrite(
                os.path.join(save_path, str(i).zfill(6) + ".jpg"), img_processed
            )
            if args.show:
...
def run():
    ...
    val_transform, trans_cfg = build_transform(
        args, trans_cfg, model_cfg["max_stride"], is_train=False
    )
