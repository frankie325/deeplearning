...
try:
    ...
    print("It seems that the COCOAPI is not installed.-1")
...
class COCODataset(Dataset):
    ...
    def pull_image(self, index):
        ...
        img_file = os.path.join(
            self.data_dir, self.image_set, "{:012}".format(img_id) + ".jpg"
        )
        image = cv2.imread(img_file)

        if self.json_file == "instances_val5k.json" and image is None:
            img_file = os.path.join(
                self.data_dir, "train2017", "{:012}".format(img_id) + ".jpg"
            )
            image = cv2.imread(img_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root",
        default="/Users/liuhaoran/Desktop/python_work/object-detection/dataset/COCO/",
        help="data root",
    )
    parser.add_argument(
    ...
    for i in range(1000):
        ...
        for box, label in zip(boxes, labels):
            ...
            image = cv2.rectangle(
                image, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 2
            )
