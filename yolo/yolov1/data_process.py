...
GL_NUMBBOX = 2
GL_NUMGRID = 7
STATIC_DATASET_PATH = r"D:\my code\yolo_data\VOC2012"
...
def img_augument(img_dir, save_img_dir, labels_dir):
    ...
    for img_name in imgs_list:
        ...
        padw, padh = (
            0,
            0,
        )  # 要记录宽高方向的padding具体数值，因为padding之后需要调整bbox的位置信息
        if h > w:
            padw = (h - w) // 2
            img = np.pad(
                img, ((0, 0), (padw, padw), (0, 0)), "constant", constant_values=0
            )
        elif w > h:
            padh = (w - h) // 2
            img = np.pad(
                img, ((padh, padh), (0, 0), (0, 0)), "constant", constant_values=0
            )
        ...
        if len(bbox) % 5 != 0:
            raise ValueError(
                "File:"
                + os.path.join(labels_dir, img_name.split(".")[0] + ".txt")
                + "——bbox Extraction Error!"
            )
...
def convert_bbox2labels(bbox):
    ...
    labels = np.zeros(
        (7, 7, 5 * GL_NUMBBOX + len(GL_CLASSES))
    )  # 注意，此处需要根据不同数据集的类别个数进行修改
    for i in range(len(bbox) // 5):
        ...
        labels[gridy, gridx, 0:5] = np.array(
            [gridpx, gridpy, bbox[i * 5 + 3], bbox[i * 5 + 4], 1]
        )
...
def create_csv_txt(
    img_dir, anno_dir, save_root_dir, train_val_ratio=0.9, padding=10, debug=False
):
    """
    TODO:
    ...
    traincsv = np.zeros(
        (n_train, GL_NUMGRID * GL_NUMGRID * (5 * GL_NUMBBOX + len(GL_CLASSES))),
        dtype=np.float32,
    )
    for i, id in enumerate(train_id):
        ...
        with open(
            os.path.join(labels_dir, "%s.txt" % img_name.split(".")[0]), "r"
        ) as f:
    ...
    testcsv = np.zeros(
        (
            n_trainval - n_train,
            GL_NUMGRID * GL_NUMGRID * (5 * GL_NUMBBOX + len(GL_CLASSES)),
        ),
        dtype=np.float32,
    )
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        STATIC_DATASET_PATH, "voc2012_forYolov1"
    )  # 保存处理后的数据(图片+标签)的文件夹
