...
第一个VGG block层：
    ...
    第一个卷积层的核大小为3x3，填充为1，步长为1，输出通道数为64，输出特征图为64x224x224
    第二个卷积层的核大小为3x3，填充为1，步长为1，输出通道数为64，输出特征图为64x224x224
    最大池化层的核大小为2x2，填充为0，步长为2。输出特征图为64x112x112

第二个VGG block层：
    包含2个卷积层，每个卷积层后面跟着一个ReLU激活函数，然后是一个最大池化层。
    第一个卷积层的核大小为3x3，填充为1，步长为1，输出通道数为128，输出特征图为128x112x112
    第二个卷积层的核大小为3x3，填充为1，步长为1，输出通道数为128，输出特征图为128x112x112
    最大池化层的核大小为2x2，填充为0，步长为2。输出特征图为128x56x56

第三个VGG block层：
    包含2个卷积层，每个卷积层后面跟着一个ReLU激活函数，然后是一个最大池化层。
    第一个卷积层的核大小为3x3，填充为1，步长为1，输出通道数为256，输出特征图为256x56x56
    第二个卷积层的核大小为3x3，填充为1，步长为1，输出通道数为256，输出特征图为256x56x56
    最大池化层的核大小为2x2，填充为0，步长为2。输出特征图为256x28x28

第四个VGG block层：
    包含2个卷积层，每个卷积层后面跟着一个ReLU激活函数，然后是一个最大池化层。
    第一个卷积层的核大小为3x3，填充为1，步长为1，输出通道数为512，输出特征图为512x28x28
    第二个卷积层的核大小为3x3，填充为1，步长为1，输出通道数为512，输出特征图为512x28x28
    最大池化层的核大小为2x2，填充为0，步长为2。输出特征图为512x14x14

第五个VGG block层：
    包含2个卷积层，每个卷积层后面跟着一个ReLU激活函数，然后是一个最大池化层。
    第一个卷积层的核大小为3x3，填充为1，步长为1，输出通道数为512，输出特征图为512x14x14
    第二个卷积层的核大小为3x3，填充为1，步长为1，输出通道数为512，输出特征图为512x14x14
